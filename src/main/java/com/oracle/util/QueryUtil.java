...
public class QueryUtil {
    ...
    public static String queryArguments(String ACCOUNT_SID,String AUTH_TOKEN,String smsContent,String to) {
        ...
        String sig = MD5(ACCOUNT_SID,AUTH_TOKEN,smsContent);//签名认证
    ...
    public static String MD5 (String... args){//动态传参
        ...
        if(args == null||args.length == 0){
            ...
            System.out.print("加密前:\t"+str.toString());
            try{
                MessageDigest digest = MessageDigest.getInstance("MD5");
                byte[] bytes = digest.digest(str.toString().getBytes());
                for(byte  b : bytes){
                    String hex = Integer.toHexString(b&0xff);//转化成十六进制
                    if(hex.length() == 1){
                        result.append("0"+hex);
                    }else{
                        result.append(hex);
        ...
        System.out.print("加密后:\t"+result.toString());
